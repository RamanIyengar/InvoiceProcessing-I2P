...
// ─── Agentic pipeline definitions (I2P Agentic Platform) ──────────────────────

const PO_STEPS = [
  { id: 'classify', name: 'Document Classification Agent', description: 'The parallel extraction from GPT Vision and Gemini successfully identified key invoice entries with detailed insights, ensuring completeness and accurate data aggregation. DocumentType identified as PO Invoice based on rule-based cross-validation from three different sources.', agents: ['Document Classification Agent', 'Document Status Agent'] },
  { id: 'extract', name: 'Document Extraction Agent', description: 'The invoice entity details were successfully extracted from Gemini, GPT-Vision, and GPT-azure, merged into a unified schema to produce complete, cross-validated results with a high level of accuracy. Confidence scores assigned per field.', agents: ['Document Extraction Agent', 'GPT Vision', 'Gemini'] },
  { id: 'quality', name: 'Document Quality Agent', description: 'The document quality check was performed successfully. Each invoice field was evaluated and analysed for detailed insights. Further detailed visual model analysis was performed on each individual data field.', agents: ['Document Quality Agent', 'Invoice Validation Agent'] },
  { id: 'patterns', name: 'Patterns Derivation', description: 'Majority Match was applied to all fields with matching score above 80% confidence. Fields with all empty values used the Pattern algorithm, falling back to Model direct confidence and preserving zero-tokens.', agents: ['Patterns Derivation Agent', 'Vendor Master Agent'] },
  { id: 'match', name: 'Invoice PO Mapping and Matching', description: 'Invoice PO found — single PO mapped to the invoice line using CAO 2-semantic match. Header net amount within tolerance of PO available amount. Pre-payment duplicate control completed. Processing completed successfully.', agents: ['PO Mapping Agent', 'Payment Validator', 'Anomaly Detection Agent'] },
]

const NON_PO_STEPS = [
  { id: 'classify', name: 'Document Classification Agent', description: 'The parallel extraction successfully identified the invoice as Non-PO. DocumentType identified from the title of the document. Classification confidence above threshold; no purchase order reference found.', agents: ['Document Classification Agent', 'Document Status Agent'] },
  { id: 'extract', name: 'Document Extraction Agent', description: 'The invoice entity details were successfully extracted and merged into a unified schema. Amount fields were standardized by removing commas and enforcing decimal precision. Invoice numbers remained unchanged.', agents: ['Document Extraction Agent', 'GPT Vision', 'Gemini'] },
  { id: 'quality', name: 'Document Quality Agent', description: 'The document quality check was performed successfully. Each invoice field was evaluated and analysed for detailed insights. Further detailed visual model analysis was performed on each individual data field.', agents: ['Document Quality Agent', 'Invoice Validation Agent'] },
  { id: 'patterns', name: 'Patterns Derivation', description: 'Majority Match was applied to all fields with matching score above 80% confidence. Fields with all empty values used the Pattern algorithm, falling back to Model direct confidence and preserving zero-tokens.', agents: ['Patterns Derivation Agent', 'Vendor Master Agent'] },
  { id: 'glcode', name: 'Coding Agent', description: 'The Coding Agent successfully GL validated and retrieved invoice data. Searched line items against the coding tool for GL codes via direct Risk Book search. All fields were found through direct search and process completed successfully.', agents: ['Coding Agent', 'Tax & Compliance Agent', 'Exception Orchestrator'] },
]

const IC_STEPS = [
  { id: 'classify', name: 'Document Classification Agent', description: 'Identified document as an intercompany invoice. DocumentType confirmed via rule-based cross-validation. Intercompany trading-partner codes validated against registered entities.', agents: ['Document Classification Agent', 'Document Status Agent'] },
  { id: 'extract', name: 'Document Extraction Agent', description: 'Intercompany invoice details extracted and cross-validated across multiple AI models. All header and line-level fields captured with high confidence scores.', agents: ['Document Extraction Agent', 'GPT Vision', 'Gemini'] },
  { id: 'quality', name: 'Document Quality Agent', description: 'The document quality check confirmed all mandatory intercompany fields are present. Entity codes, amounts, and currency are validated against the ICE reconciliation system.', agents: ['Document Quality Agent', 'Invoice Validation Agent'] },
  { id: 'patterns', name: 'Patterns Derivation', description: 'Majority Match applied to all fields. Historical intercompany settlement patterns cross-referenced for predictive matching guidance.', agents: ['Patterns Derivation Agent', 'Vendor Master Agent'] },
  { id: 'icmatch', name: 'Invoice PO Mapping and Matching', description: 'Predictive IC & Royalty Settlement agent matches both sides of the intercompany posting in real time and reconciles against the ICE system across affiliated entities.', agents: ['PO Mapping Agent', 'IC Reconciliation Agent', 'Anomaly Detection Agent'] },
]

const ROYALTY_STEPS = [
  { id: 'classify', name: 'Document Classification Agent', description: 'Identified document as a royalty invoice. Contract reference detected in the document header. DocumentType confirmed as royalty settlement via rule-based validation.', agents: ['Document Classification Agent', 'Document Status Agent'] },
  { id: 'extract', name: 'Document Extraction Agent', description: 'Royalty lines, rates, and contract references extracted with high accuracy. Amount fields standardized and validated against contract terms.', agents: ['Document Extraction Agent', 'GPT Vision', 'Gemini'] },
  { id: 'quality', name: 'Document Quality Agent', description: 'Document quality check confirmed all mandatory royalty fields and contract references are present. Rate basis and calculation methodology validated.', agents: ['Document Quality Agent', 'Invoice Validation Agent'] },
  { id: 'patterns', name: 'Patterns Derivation', description: 'Majority Match applied. Historical royalty settlement patterns and contract rate history cross-referenced for deviation detection.', agents: ['Patterns Derivation Agent', 'Vendor Master Agent'] },
  { id: 'royaltycheck', name: 'Coding Agent', description: 'GenAI compares each royalty invoice line against abstracted contract terms (rate, basis, clauses). The Coding Agent validated GL codes against the royalty chart of accounts and flagged genuine rate deviations for review.', agents: ['Coding Agent', 'Predictive Royalty Agent', 'Tax & Compliance Agent'] },
]

const FREMANTLE_STEPS = [
  { id: 'classify', name: 'Document Classification Agent', description: 'Identified document as a production milestone invoice. DocumentType confirmed as PO Invoice based on cross-validation from three different extraction sources.', agents: ['Document Classification Agent', 'Document Status Agent'] },
  { id: 'extract', name: 'Document Extraction Agent', description: 'Header and line-level production fields extracted and cross-validated across multiple AI models. Milestone references and production codes captured with high confidence.', agents: ['Document Extraction Agent', 'GPT Vision', 'Gemini'] },
  { id: 'quality', name: 'Document Quality Agent', description: 'Document quality check performed successfully. Production milestone fields, PO reference, and delivery confirmation validated against project structure.', agents: ['Document Quality Agent', 'Invoice Validation Agent'] },
  { id: 'patterns', name: 'Patterns Derivation', description: 'Majority Match applied to all fields. Production milestone patterns cross-referenced against contract deliverables and historical project billing.', agents: ['Patterns Derivation Agent', 'Vendor Master Agent'] },
  { id: 'match', name: 'Invoice PO Mapping and Matching', description: 'Where the milestone is described in the production contract rather than structured in the ERP, GenAI reads the contract clause, confirms the condition is met, and completes the 3-way match — clearing the invoice without an analyst.', agents: ['PO Mapping Agent', 'GenAI Milestone Interpreter', 'Payment Validator'] },
]
